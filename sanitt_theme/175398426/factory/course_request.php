<?php 
	...
	function getRequestID(){
		$directory = "course_request/log/";
		$files = glob($directory . "*.txt");
		if ( $files !== false )
		{
		    $filecount = count( $files );
		    $requestID = $filecount + 1;
	...
	function registration_sendMail($from, $to, $subject, $body){
		...
		$mail->setFrom('anfrage@sanitt.de', 'Sanitt Anfrage');
		foreach ($to as $single_email){
			$mail->addAddress($single_email);     // Add a recipient
		}
		// $mail->addAddress('ellen@example.com');               // Name is optional
	...
	function saveUserData($requestID,$admin_email){
		
		$myfile = fopen("course_request/log/logfile_".time()."_ID".$requestID.".txt", "wb") or die("Unable to open file!");
		...
		registration_sendMail("kunde@sanitt.de",$admin_email,$_POST['course_data']['topic'].': '.$_POST['course_data']['contents'],"Hello Admin!\nYou have a new request!\n Here is the logfile:\n".$pretty);
	}
	function zipAndDelete($admin_email){
		// Get real path for our folder
		$rootPath = realpath('course_request/log/');

		// Initialize archive object
		$zip_name = 'course_request/backup_requests_'.time().'.zip';
		$zip = new ZipArchive();
		...
		backup_sendMail('backup@sanitt.de',$admin_email,'New Backup',"Hello Admin!\n Here is a backup.zip containing the last recent request files.",$zip_name);
	}
	...
	if (isset($_POST['key']) && $_POST['key'] == $key) {
		...
		saveUserData($requestID,$admin_email);
		if (isset($_POST['zip']) && $_POST['zip'] == true && $requestID > 100) {
