...
.controller('courseCtrl', ['$scope','$http',function($scope, $http){
	jQuery(function($) {
		...
		// ###############
		// removeLastMember()
		// @description: 	Removes last entry of $scope.register_data.course_members[] IF more then 1
		// 					
		$scope.removeLastMember = function(){
			if($scope.register_data.course_members.length > 1){
				$scope.register_data.course_members.pop();
		...
		$scope.submitForm = function(validation){
			...
			if (validation){
				...
				$.ajax({
							type: "POST",
							url: wp_templateUrl + '/175398426/factory/course_registration.php',
		...
		$scope.submitFormRequest = function(validation){
			...
			console.log("check data..");
			if (validation){
				if ($scope.request_data.alternative_billing == false) {$scope.request_data.billing = $scope.request_data.customer};
				$post_data = $scope.request_data;
				console.log($post_data);
				// alert("Returned data: \n"+JSON.stringify($scope.register_data, null, 2));
				// HIER DATA HANDLER
				$post_data = $scope.request_data;
				...
				$.ajax({
							type: "POST",
							url: wp_templateUrl + '/175398426/factory/course_request.php',
							data: $post_data, // serializes the form's elements.
							success: function(response){
				  				console.log("Service request successful.");
				  				console.log("Response: "+response);
