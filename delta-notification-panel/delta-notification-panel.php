<?php
/**
* Plugin Name: Multiple Notification Panels
* Plugin URI: http://deltachi.de/notification_panels
* Description: [1]Create a panel with heading, body and a button that can be programmed with javascript. Choose a color and set a timer for the whole panel to animate-in. [2]Create a panel that animates in as the user scrolls to that panel.
* Version: 1.0 
...
class wp_timed_notification_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo '
					<article class="green slide-in" style="max-height: 300px; overflow: hidden; background-color:'; echo $background_color; echo ';">
						<div class="container">
...
//#################################################################
// Creating the Scroll Notification Widget 
//#################################################################
class wp_scroll_notification_widget extends WP_Widget {

	function __construct() {
		parent::__construct(
		// Base ID of your widget
		'wp_scroll_notification_widget', 

		// Widget name will appear in UI
		__('Scroll Triggered Notification Widget', 'wp_scroll_notification_widget_domain'), 

		// Widget description
		array( 'description' => __( 'Panel with heading and body. Scroll based slide-in.', 'wp_scroll_notification_widget_domain' ), ) 
		);
		//loads colorpicker API css and js
		// add_action( 'load-widgets.php', array(&$this, 'my_custom_load') );
	}
 ...
 //    }
	...
	public function widget( $args, $instance ) {
		...
		// This is where you run the code and display the output
		echo '
				<div class="caution-container" class="container">
					<div class="caution animate">
						<h3>';
		echo 				$title;
		echo			'</h3>
						<p>';
	...
	// Widget Backend 
	public function form( $instance ) {
		$title = isset( $instance['title'] ) ? esc_attr( $instance['title'] ) : __( 'New title', 'wp_scroll_notification_widget_domain' );
		// $body = isset( $instance['body'] ) ? esc_attr( $instance['body'] ) : __( 'New body', 'wp_scroll_notification_widget_domain' );
...
} // Class wp_scroll_notification_widget ends here
...
// Register and load the widget
function wp_scroll_notifications_load_widgets() {
	register_widget( 'wp_scroll_notification_widget' );
}
add_action( 'widgets_init', 'wp_scroll_notifications_load_widgets' );
...
function wp_timed_notifications_load_styles()
{
    ...
    wp_register_style( 'delta-notification-panel-style', plugins_url( '/css/delta-notification-panel.css', __FILE__ ), array(), '20120208', 'all' );
    // or
    // Register the style like this for a theme:
    // wp_register_style( 'custom-styles', get_template_directory_uri() . '/css/custom-styles.css', array(), '20120208', 'all' );
 
    // For either a plugin or a theme, you can then enqueue the style:
    wp_enqueue_style( 'delta-notification-panel-style' );
}
